...
export default function GuardianNotificationDetailPage() {
  ...
  useEffect(() => {
    if (userId) loadConfigs();
  }, [userId]);

  const loadConfigs = async () => {
    setLoading(true);
    try {
      const list = await getGuardianNotificationConfig(userId);
  ...
  const handleToggle = async (channel: string, enabled: boolean) => {
    ...
    try {
      const updated = await updateGuardianNotificationConfig(userId, channel, {
        enabled,
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const updated = await updateGuardianNotificationConfig(userId, editChannel.key, {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <Link
          href="/admin/guardian-notifications"
