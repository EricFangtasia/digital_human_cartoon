...
export class AudioRecoder {
  ...
  private _silenceNode: GainNode | null = null;
  ...
  async start() {
    ...
    this._silenceNode = this._audioContext.createGain();
    this._silenceNode.gain.value = 0;
    this._audioWorkletNode.port.onmessage = (event) => {
      this.processAudioData(event.data);
    }
    this._sourceNode.connect(this._audioWorkletNode);
    this._audioWorkletNode.connect(this._silenceNode);
    this._silenceNode.connect(this._audioContext.destination);
  ...
  flush(): Uint8Array | null {
    ...
    return new Uint8Array(this._audioBuffer.splice(0, this._audioBuffer.length));
  }
  ...
  stop(): void {
    ...
    if (this._silenceNode) {
        this._silenceNode.disconnect();
        this._silenceNode = null;
    }
